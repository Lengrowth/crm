...
| `lenerp_core` | Private `https://github.com/Len-OS/lenerp_core.git`; protected `main` candidate `a7e47208baf6583295f5f2632f4787262cd3f475` (`0.2.0`) | C01–C08 synthetic DocTypes, persisted C08 dashboard outputs/alerts, complete commercial/inventory/asset-maintenance seed, real role principals, workflow, reports, print path, read-only status/reset, and app-owned ERP accessibility remediation; isolated ERP staging installation remains candidate-bound |
...
- The next protected C01–C08 staging run is candidate-bound to the exact CRM
  ...
  `lenerp_core` archive pinned at `a7e47208baf6583295f5f2632f4787262cd3f475`.
  It must include persisted commercial/inventory/asset-maintenance counts,
  C08 dashboard/alert outputs, real role-principal denials, ERP browser/print/export/accessibility evidence,
  and read-only zero-count reset evidence. This remains operator-validation
  staging only: no Champion acceptance, real-data migration, Project Start
  approval, production activation, or final-domain cutover is claimed.
...
- HRMS source identity is recorded at upstream tag `v15.64.1`, commit
  `e68a3deaa95ae5b2c3d743297d0a4ab505733fc1`, licensed under GNU GPL v3, but
  the pin is blocked: its official `after_install` fixture still references
  the removed Frappe `Expense Claim Type` controller on the declared Frappe
  v15.119.1 / ERPNext v15.120.0 baseline. The authoritative dependency record
  is `ops/staging/application-dependencies.json`.
- The resolver derives platform applications separately from effective module
  ...
  preflight now fail closed before mutation while compatibility is blocked.
  Staging and production gates have not passed.
...
The next protected main run is the authoritative candidate-bound staging record for the CRM source and LenERP Core `a7e47208baf6583295f5f2632f4787262cd3f475`. It must include the durable browser evidence bundle, documentation snapshot/hash, C08 dashboard outputs, ERP role/print/export evidence, reviewed accessibility checks, responsive checks, language audit, and post-demo cleanup. Independent review remains the next gate; no production cutover or real customer data is authorized.
