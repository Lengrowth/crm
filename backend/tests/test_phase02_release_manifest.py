...
def test_candidate_manifest_records_verified_hrms_and_exact_runtime_baseline(tmp_path: Path) -> None:
    ...
    baseline = json.loads(
        (candidate / "ops" / "staging" / "release-runtime-baseline.json").read_text(encoding="utf-8")
    )
    assert manifest["installed_apps"]["lenerp_core"]["commit"] == baseline["runtime_apps"]["lenerp_core"]["commit"]
