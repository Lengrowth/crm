...
def test_erp_identity_boundary_is_code_based_and_secret_safe():
    ...
    assert "_STATE_COOKIE" in SSO
    assert "browser_binding" in SSO
    assert "advisory_lock" in SSO
    assert "DuplicateEntryError" in SSO
    assert "exchange_handle" in SSO
...
def test_hooks_include_bidirectional_navigation_and_local_login_guard():
    ...
    assert "data-lenerp-control-plane" in BUNDLE
    assert "Return to LenERP Control Plane" in BUNDLE
